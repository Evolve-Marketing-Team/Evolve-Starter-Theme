...
// Include node_modules for sass paths
const sassPaths = [
	'node_modules'
 ...
 *    7. Injects CSS or reloads the browser via browserSync
 */
gulp.task('styleBlocks', () => {
	return gulp
		.src(config.styleBlocksSRC, {allowEmpty: true})
		.pipe(plumber(errorHandler))
		.pipe(sourcemaps.init())
		.pipe(
			sass({
				includePaths: blockSassPaths,
				errLogToConsole: config.errLogToConsole,
				outputStyle: config.outputStyle,
				precision: config.precision,
				fiber: Fiber
		...
		.pipe(sourcemaps.write('./'))
		.pipe(lineec()) // Consistent Line Endings for non UNIX systems.
		.pipe(gulp.dest(config.styleBlocksDest))
		.pipe(filter('**/*.css')) // Filtering stream to only css files.
		.pipe(mmq({log: true})) // Merge Media Queries only for .min.css version.
		.pipe(browserSync.stream()) // Reloads style.css if that is enqueued.
		.pipe(rename({suffix: '.min'}))
...
editorSRC = './src/scss/editor-styles.scss',
editorDEST = './assets/css'

gulp.task( 'editor-styles', () => {
	return gulp
		.src( editorSRC, { allowEmpty: true })
		.pipe( plumber( errorHandler ) )
		.pipe( sourcemaps.init() )
		.pipe(
			sass({
...
wpLoginSRC = './src/scss/wplogin.scss',

gulp.task( 'wplogin-style', () => {
	return gulp
		.src( wpLoginSRC, { allowEmpty: true })
		.pipe( plumber( errorHandler ) )
		.pipe( sourcemaps.init() )
		.pipe(
			sass({
